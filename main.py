...
#Definiendo alto y ancho de pantalla
ancho_pantalla = 500
alto_pantalla = 650
...
musica_inicio = pygame.mixer.Sound("assets/inicio.wav")
musica_inicio.play(-1)
musica_inicio.set_volume(0.7)
sonido_salto = pygame.mixer.Sound("assets/salto.wav")
sonido_muerte = pygame.mixer.Sound("assets/muerto.wav")

#Definiendo fuente
fuente_GO = pygame.font.SysFont('Georgia', 20)
fuente_puntaje = pygame.font.SysFont('Digital-7 Mono', 70)
...
class Mario(pygame.sprite.Sprite):
    ...
    #Función para salto de Mario
    def jump(self):
        self.speed = -velocidad    
    
    #Función para sonido del salto
...
#Clase para imprimir el Game Over
...
#Función para reseteo del juego
def reset_game():
    grupo_tubo.empty()
    mario.rect.x = 100
...
juego_terminado = juego_terminado((ancho_pantalla/3) - 20, (alto_pantalla/2) - 30, img_game_over)
...
while run:
    ...
    if mario.rect.bottom > posicion_piso - 10:
        game_over = True
        vuelo = False
    ...
    if game_over == True:
        ...
        if puntaje >= ultimo_puntaje:
            textoFelicitacion("¡Felicidades has superado el Mejor Puntaje!", fuente_GO, blanco, 60, int((alto_pantalla / 5) * 3))
            textoMejorPuntaje("El Mejor Puntaje era de: " + str(ultimo_puntaje), fuente_GO, blanco, 140, int((alto_pantalla / 5) * 3.5))
            textoPuntaje("Tu Puntaje fue de: " + str(puntaje), fuente_GO, blanco, 160, int((alto_pantalla / 5) * 4))
        else: 
            textoFelicitacion("No has superado el Mejor Puntaje :(", fuente_GO, blanco, 90, int((alto_pantalla / 5) * 3))
            textoMejorPuntaje("El Mejor Puntaje es: " + str(ultimo_puntaje), fuente_GO, blanco, 150, int((alto_pantalla / 5) * 3.5))
            textoPuntaje("Tu Puntaje fue de: " + str(puntaje), fuente_GO, blanco, 155, int((alto_pantalla / 5) * 4))
